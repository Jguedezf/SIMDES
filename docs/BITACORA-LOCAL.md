...
### 8. Revisión exhaustiva de calidad (Pressman/McCall), patrones de diseño, base de datos, validaciones/CRUD/gráficas (2026-09-13)

**Patrones de diseño documentados por primera vez (sección 8.6 del informe):** Johanna señaló, con razón, que nunca se había especificado qué patrón(es) de diseño de software se usaron — se investigó el código real (no se inventaron nombres) y se documentaron 7 patrones con evidencia concreta: Contenedor/Presentación (Server+Client Component en `contenedor/[id]`, `reportes`, `contenedores/nuevo`), Factory Method (`crearClienteServidor`/`crearClienteNavegador`), Guard Clause (`exigirRol`), Adapter/Wrapper de plataforma (`mapa-*-wrapper.tsx` para el límite de `ssr:false` de Next 16), Componente Controlado (`ModalConfirmacion`/`Toast`), Pipes-and-Filters con manejo de error no bloqueante (pipeline de n8n), y Mínimo Privilegio como patrón de encapsulamiento (`actualizar_mi_nombre`). Se agregaron también ejemplos concretos de SOLID/DRY/fail-safe-defaults, reemplazando la mención genérica de "SOLID" que solo existía como requisito del DoD sin ejemplos reales.

**Factores de calidad de Pressman/McCall (sección 4.8, nueva):** el informe ya tenía un plan de calidad de *proceso* (IEEE 730, PHVA) pero no el modelo de *factores de calidad del producto* que pide explícitamente el punto "d" de los entregables. Se agregó una tabla de los 11 factores (Corrección, Fiabilidad, Eficiencia, Integridad, Usabilidad, Mantenibilidad, Flexibilidad, Testabilidad, Portabilidad, Reusabilidad, Interoperabilidad), cada uno contrastado contra evidencia real ya producida en el proyecto (no una declaración de intención) — con honestidad explícita sobre el hueco real: no hay suite de pruebas unitarias automatizadas, la verificación real es funcional/end-to-end (simulador + Playwright + SQL directo).

**Segunda re-verificación de la base de datos (sección 9.6, nueva):** se volvió a correr el linter de seguridad/performance de Supabase (no se asumió de memoria el estado del 12/09). Sin hallazgos nuevos — los mismos ya triados siguen aceptados con la misma justificación. Se revisó explícitamente por primera vez `log_automatizacion` con RLS habilitado sin policy: es la postura correcta (deniega todo acceso vía PostgREST; n8n escribe con `service_role key`, que ignora RLS), no un descuido.

**Auditoría de validaciones/CRUD/animaciones/gráficas:** se revisaron los 5 `<form>` conocidos (login, alta de contenedor, perfil-nombre, perfil-password, filtro GET de `/contenedores`) — los 2 fixes del 12/09 (whitelist de filtros, límite de 120 caracteres) siguen aplicados, sin gaps nuevos. CRUD de cada entidad (contenedores, alertas, perfiles) confirmado sin huecos reales dado su diseño (alertas y perfiles ajenos son de escritura acotada a propósito, no un CRUD completo por decisión de alcance ya documentada). Animaciones: todas las acciones asíncronas (guardar nombre, cambiar password, resolver alerta, cambiar estado, eliminar/restaurar contenedor) ya tienen estado de carga (`guardando`/`enviando`) — no se encontró ninguna acción que falle en silencio sin feedback visual.

**2 mejoras reales encontradas y corregidas en las gráficas de Recharts** (no cosméticas — violaban una regla concreta de la skill `dataviz` o le faltaba información de dominio real):
1. `grafico-historial.tsx` (barras apiladas de 3 series: pendiente/enviada/resuelta) no tenía ninguna leyenda — se agregó una fila de indicadores de color, igual al patrón ya usado en `grafico-alertas.tsx` (el donut).
2. El gráfico de línea de "Historial de llenado" (`detalle-cliente.tsx`) no marcaba el umbral crítico del 85% (`NIVEL_UMBRAL_ALTO`, `src/lib/nivel.ts`) que es central a todo el sistema — se agregó una `ReferenceLine` roja punteada. De paso se encontró que el eje X solo mostraba hora sin fecha, lo que hacía ver "desordenadas" lecturas de días distintos con horas parecidas (los datos SÍ estaban bien ordenados por `timestamp`, era solo la etiqueta) — se corrigió para mostrar día/mes cuando el historial abarca más de un día.
